function pillow_MID_hist(fname)
...
load(fname)
disp(fname)
outname = [fname(1:end-4) 'fitted_pillow_MID_hist.mat'];
if exist(outname)
...
%% Add hist
y = y(:);
hist_term = zeros(size(y,1),hist_max);
for ii=1:hist_max
    hist_term(ii+1:end,ii) = y(1:end-ii);
end
X = [X hist_term];
%%
disp(size(X))
...
%% ===================== Put data in struct ============= %
output.yhat = yhat;
output.yhat_avg = yhat_avg;
output.ppcbf = ppcbf;
output.ppcbf_avg = ppcbf_avg;
output.fstructCBF = fstructCBF;
output.pp0 = pp0;
output.R = R;
%%
save(outname)
